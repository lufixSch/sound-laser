...
class BluetoothAudio:
    ...
    def _worker_loop(self):
        ...
        while self.wlt:
            ...
            if not self.channel:
                ...
                continue
            logging.info('HSP/HFP found on RFCOMM channel ' +
                         str(self.channel))
            ...
            try:
                ...
                logging.warning(
                    'Service level connection disconnected: ' + str(e))
                time.sleep(self.HFP_TIMEOUT)
            self._cleanup()
        ...
        while self.wlt:
            ...
            if not self.audio:
                if audio_time < time.time():
                    if sevice_notice:
                        logging.warning(
                            'Service connection timed out, try audio anyway...')
                        sevice_notice = False
                    self._connect_audio()
    ...
    def _connect_service_level(self):
        ...
        try:
            ...
            hfp.close()
            logging.warning(
                'Failed to establish service level connection: ' + str(e))
            return
        ...
        try:
            if self.resample:
                ...
                for i in range(0, len(data), 4):
                    # two samples of signed 16 bit le
                    val1, val2 = struct.unpack_from('<hh', data, i)
    ...
    def beep(self, length_ms=300, frequency=1000.0, amplitude=0.5):
        ...
        for i in range(0, length):
            val = 32767.0 * amplitude * \
                math.sin(2.0 * math.pi * float(i % period) / period)
...
def main():
    ...
    if not bluetooth.is_valid_address(sys.argv[1]):
        ...
        return
    hf = BluetoothAudio(
        sys.argv[1], BluetoothAudio.AUDIO_16KHZ_SIGNED_16BIT_LE_MONO)
