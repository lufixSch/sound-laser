...
\end{align}
%
Given the connection between distance, time and velocity. This equation can be used to calculate the phaseshift $\Phi$ between those two waves.
%
\begin{align}
...
\end{align}
%
This formula can also be used for a whole array of emitters, where $\Phi(\varphi)$ describes the phaseshift between two adjacent emitters. To get the phaseshift of the $n$th speaker relative to the first one ($n=0$) the phaseshifts $\Phi(\varphi)$ just need to be summed up.
%
\begin{align}
...
As shown in equation \ref{eq:theory:beam:single_phaseshift} the phaseshift between two emitters depends on the distance between them. Therefore the beamforming characteristics can be easily influenced by this. The ideal distance is half of the wavelength of the transmitted signal. With this spacing the two waves cancel each other out at exactly $90^\circ$ as shown in figure \dots and \dots.\p
%
Figure \dots shows the beamforming characteristics of a phased array with $2$ emitters and a distance of \dots, \dots, \dots and \dots. The graph shows that using a smaller distance than $\frac{1}{2} \lambda$ makes the beamforming effect slowly disappear while a higher distance introduces a side beam at $\pm 90^\circ$. If the distance is further increased, the side beams split up and a new one emerges at $\pm 90^\circ$.
