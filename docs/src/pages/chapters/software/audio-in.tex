...
%
The \lstcpp{PCM} class is initialized in \lstcpp{AudioProcessor::configure()}. This instance is then used in the method \lstcpp{record()} shown in listing \dots. \lstcpp{record()} creates a loop in wich a set of samples is read into a buffer and then pushed into the \lstcpp{AudioProcessor::samples} queue. The method can be executed directly or by execution \lstcpp{record_thread()} which creates a new thread for the loop.
